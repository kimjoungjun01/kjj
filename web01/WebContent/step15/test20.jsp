...
<c:if test="name=='유관순'">유관순1111</c:if><br>
<c:if test="${name=='유관순'}">유관순2222</c:if><br>
<c:if test="${age==20}">20살</c:if><br>
<c:if test="${working==true}">재직중11</c:if><br>
<c:if test="${working}">재직중22</c:if><br>

<%--이프의 결과를 보관소에 저장하는 법 --%>
<c:if test="${working}" var="result">재직중3</c:if>
####${pageScope.result}####<br>
