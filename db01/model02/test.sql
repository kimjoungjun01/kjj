...
-- 학생
DROP TABLE IF EXISTS `MY_SCHEMA`.`STUDENT` RESTRICT;
...
CREATE TABLE `MY_SCHEMA`.`LECT` (
	...
	`GOV_SUPP` CHAR(1) NOT NULL COMMENT '정부지원여부', -- 정부지원여부
...
CREATE TABLE `MY_SCHEMA`.`STUDENT` (
	...
	`BRTH_DT` <데이터 타입 없음> NULL     COMMENT '생년월일', -- 생년월일
	`WORK`    <데이터 타입 없음> NOT NULL COMMENT '재직여부' -- 재직여부
)
COMMENT '학생';
...
ALTER TABLE `MY_SCHEMA`.`CURR_SUBJ`
	ADD CONSTRAINT `PK_CURR_SUBJ` -- 교육과정번호 기본키
		PRIMARY KEY (
			`EDNO`, -- 교육과정번호
			`SBNO`  -- 과목번호
		);
...
-- 학생
ALTER TABLE `MY_SCHEMA`.`STUDENT`
	ADD CONSTRAINT `FK_MEMB_TO_STUDENT` -- 회원 -> 학생
		FOREIGN KEY (
			`MNO` -- 학생번호
		)
...
-- 교육과정번호
ALTER TABLE `MY_SCHEMA`.`CURR_SUBJ`
	ADD CONSTRAINT `FK_CURR_TO_CURR_SUBJ` -- 교육과정 -> 교육과정번호
		FOREIGN KEY (
			`EDNO` -- 교육과정번호
		)
		REFERENCES `MY_SCHEMA`.`CURR` ( -- 교육과정
...
-- 교육과정번호
ALTER TABLE `MY_SCHEMA`.`CURR_SUBJ`
	ADD CONSTRAINT `FK_SUBJ_TO_CURR_SUBJ` -- 과목 -> 교육과정번호
		FOREIGN KEY (
			`SBNO` -- 과목번호
		)
...
-- 강의수강생
ALTER TABLE `MY_SCHEMA`.`LECT_STUD`
	ADD CONSTRAINT `FK_STUDENT_TO_LECT_STUD` -- 학생 -> 강의수강생
		FOREIGN KEY (
			`MNO` -- 학생번호
		)
